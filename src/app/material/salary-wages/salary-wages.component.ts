...
import {
  SalaryMapResponseData,
  SalaryEntry,
} from '../../models/SalaryMapResponse.model';
...
export class SalaryWagesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.salaryMap$.subscribe((data: any[] | null | undefined) => {
      ...
      data.forEach((entry: SalaryEntry) => {
        ...
        // Previous months
        if (
          entry.previousSalaryWagesDetails &&
          Array.isArray(entry.previousSalaryWagesDetails)
        ) {
  ...
  editSalaryWagesFull(processName: string) {
    const fullEntry = this.salaryMapRawData.find(
      (entry) => entry.processName === processName
    );
